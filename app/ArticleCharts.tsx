...
// 站点主题色板（柱状图用）—— 深色背景，取高亮度色
const COLORS = ['#7fdbb6', '#b8c4cc', '#f59e0b', '#60a5fa', '#f87171', '#c4b5fd'];
function palette(i: number): string {
  return COLORS[i % COLORS.length];
}

// 饼图专用高对比度色板（深色底，切片间需明显区分）
const PIE_COLORS = ['#7fdbb6', '#f59e0b', '#60a5fa', '#f87171', '#c4b5fd', '#5eead4'];

// 深色主题下的文字/网格颜色
const TEXT_COLOR = '#9a9a9e';
 ...
 *
 * 设计要点：
 * - 饼图：6 色高对比度色板 + 深色描边，区分切片清晰；hover 偏移 8px。
      ...
      if (chartType !== 'pie') {
        chartOptions.scales = {
          ...
          y: {
            ...
            grid: { color: GRID_COLOR },
            title: hasOutlier
              ? { display: true, text: '主量纲', color: TEXT_COLOR }
              : undefined,
          },
        };
        if (hasOutlier) {
          chartOptions.scales.y1 = {
            ...
            title: { display: true, text: '次量纲', color: TEXT_COLOR },
          };
        }
      }
